        ...
        if (message && userProfile && toggleState !== undefined) {
            ...
            try {
                ...
                if (mode === 'natural') {
                    ...
                    prompt = await buildNaturalIntentPrompt(message, chatHistory, userProfile, jobContext, toggleState);
                } else if (mode === 'analysis') {
                    // For analysis mode, build the detailed analysis prompt
                    prompt = await buildDetailedAnalysisPrompt(message, chatHistory, userProfile, jobContext, toggleState);
...
async function summarizeChatHistory(chatHistory, maxMessages = CHAT_HISTORY_CONFIG.KEEP_RECENT_MESSAGES) {
...
async function buildUserPromptServerSide(message, userProfile, jobContext, chatHistory, toggleState, mode) {
    ...
    if (chatHistory && Array.isArray(chatHistory) && chatHistory.length > CHAT_HISTORY_CONFIG.SUMMARIZATION_THRESHOLD) {
        ...
        processedChatHistory = await summarizeChatHistory(chatHistory, CHAT_HISTORY_CONFIG.KEEP_RECENT_MESSAGES);
    }
...
Respond naturally and helpfully based on the user's request.`;
        ...
        return await buildDetailedAnalysisPrompt(message, chatHistory, userProfile, jobContext, toggleState);
    }
...
async function buildNaturalIntentPrompt(message, chatHistory, userProfile, jobContext, isProfileToggleOff) {
...
async function buildDetailedAnalysisPrompt(message, chatHistory, userProfile, jobContext, toggleState) {
