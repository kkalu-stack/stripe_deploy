...
//
// ✅ NEW FEATURE: Chat History Summarization
// - Automatically summarizes chat history when it exceeds the threshold (default: 10 messages)
// - Keeps recent messages intact while summarizing older conversation
// - Reduces token usage and prevents code exposure from long chat histories
// - Configurable via CHAT_HISTORY_CONFIG object
...
const CHAT_HISTORY_CONFIG = {
    ...
    // Maximum length for summary (in words)
    MAX_SUMMARY_WORDS: 200,
    // Enable/disable summarization feature
    ENABLED: true
};
...
// ✅ CLEAN AI RESPONSE: Remove unwanted tags and prefixes
function cleanAIResponse(response) {
    if (!response || typeof response !== 'string') {
        return response;
    }
    
    // Remove decision tags at the beginning of the response
    const cleanedResponse = response.replace(/^\[(ANALYSIS|RESUME_GENERATION|COVER_LETTER_GENERATION|SHOW_RESUME|CONVERSATION|CLARIFICATION)\]\s*/i, '');
    
    // Remove any other unwanted prefixes that might appear
    const additionalCleaning = cleanedResponse.replace(/^\[(SYSTEM|AI|ASSISTANT|RESPONSE)\]\s*/i, '');
    
    // Log if any cleaning was performed
    if (cleanedResponse !== response) {
        console.log('🧹 [RESPONSE CLEANING] Removed decision tag from response:', {
            originalLength: response.length,
            cleanedLength: cleanedResponse.length,
            removedTag: response.match(/^\[(ANALYSIS|RESUME_GENERATION|COVER_LETTER_GENERATION|SHOW_RESUME|CONVERSATION|CLARIFICATION)\]/i)?.[1]
        });
    }
...
app.post('/api/generate', cors(SECURITY_CONFIG.cors), authenticateSession, async (req, res) => {
    try {
        ...
        if (message && userProfile && toggleState !== undefined) {
            ...
            try {
                ...
                if (mode === 'natural') {
                    ...
                    prompt = await buildNaturalIntentPrompt(message, chatHistory, userProfile, jobContext, toggleState, sessionId);
                } else if (mode === 'analysis') {
                    // For analysis mode, build the detailed analysis prompt
                    prompt = await buildDetailedAnalysisPrompt(message, chatHistory, userProfile, jobContext, toggleState, sessionId);
        ...
        // ✅ CLEAN RESPONSE: Remove decision tags and other unwanted prefixes
        responseContent = cleanAIResponse(responseContent);
...
async function summarizeChatHistory(chatHistory, maxMessages = CHAT_HISTORY_CONFIG.KEEP_RECENT_MESSAGES, sessionId = null) {
    // Check if summarization is enabled
    if (!CHAT_HISTORY_CONFIG.ENABLED) {
        console.log('📝 [CHAT HISTORY] Summarization disabled, using truncation');
        return chatHistory.slice(-maxMessages);
    }
    
    if (!chatHistory || !Array.isArray(chatHistory) || chatHistory.length <= CHAT_HISTORY_CONFIG.SUMMARIZATION_THRESHOLD) {
        return chatHistory; // Return original if no summarization needed
    }
    
    // ✅ CRITICAL: Check for session boundary markers
    // Look for job description messages that indicate session boundaries
    const hasJobDescriptionMarker = chatHistory.some(msg => 
        msg.content && (
            msg.content.includes('[JOB_DESCRIPTION_START]') ||
            msg.content.includes('job description') ||
            msg.content.includes('position description') ||
            msg.content.includes('we are looking for')
        )
    );
    
    if (hasJobDescriptionMarker) {
        console.log('🔄 [CHAT HISTORY] Job description detected in history, checking for session boundaries');
        
        // Find the last job description marker
        let lastJobDescriptionIndex = -1;
        for (let i = chatHistory.length - 1; i >= 0; i--) {
            ...
            if (msg.content && (
                ...
                lastJobDescriptionIndex = i;
                break;
            }
        }
        
        // If we found a job description marker, only summarize messages after it
        if (lastJobDescriptionIndex >= 0) {
            const messagesAfterJobDescription = chatHistory.slice(lastJobDescriptionIndex);
            console.log('🔄 [CHAT HISTORY] Found job description at index', lastJobDescriptionIndex, 'messages after:', messagesAfterJobDescription.length);
            
            // Only summarize if there are enough messages after the job description
            if (messagesAfterJobDescription.length > CHAT_HISTORY_CONFIG.SUMMARIZATION_THRESHOLD) {
                console.log('🔄 [CHAT HISTORY] Summarizing only messages after job description');
                return await summarizeChatHistory(messagesAfterJobDescription, maxMessages, sessionId);
            } else {
                console.log('📝 [CHAT HISTORY] Not enough messages after job description to summarize');
                return chatHistory;
            }
        }
    }
    ...
    try {
        // Split history into recent messages and older messages
        ...
        console.log('🔄 [CHAT HISTORY] Starting summarization:', {
            totalMessages: chatHistory.length,
            olderMessages: olderMessages.length,
            recentMessages: recentMessages.length,
            willKeep: maxMessages
        });
        
        // Create a summary prompt for the older messages
        const olderHistoryText = olderMessages.map(msg => 
            `${msg.role === 'user' ? 'User' : 'Assistant'}: ${msg.content}`
        ).join('\n');
        
        const summaryPrompt = `Please provide a concise summary of the following conversation history. Focus on key topics, decisions made, and important context that would be relevant for continuing the conversation. Keep it under ${CHAT_HISTORY_CONFIG.MAX_SUMMARY_WORDS} words:
...
Summary:`;
        ...
        const summaryResponse = await callOpenAI(summaryPrompt, 'natural', false, null, {}, [], false, 'Summarize conversation history');
        
        if (summaryResponse && summaryResponse.success) {
            // Create a summary message
            const summaryMessage = {
                role: 'system',
                content: `[CONVERSATION SUMMARY] ${summaryResponse.response}`
            };
            
            const result = [summaryMessage, ...recentMessages];
            
            console.log('✅ [CHAT HISTORY] Summarization completed:', {
                originalLength: chatHistory.length,
                summarizedLength: result.length,
                summaryLength: summaryResponse.response.length,
                tokenReduction: Math.round(((chatHistory.length - result.length) / chatHistory.length) * 100) + '%'
            });
            
            // Return summary + recent messages
            return result;
        } else {
            // Fallback: just return recent messages if summarization fails
            console.warn('⚠️ [CHAT HISTORY] Summarization failed, using recent messages only');
            return recentMessages;
        }
    } catch (error) {
        console.error('❌ [CHAT HISTORY] Error during summarization:', error);
...
async function buildUserPromptServerSide(message, userProfile, jobContext, chatHistory, toggleState, mode, sessionId = null) {
    ...
    // ✅ NEW: Use summarized chat history instead of truncation
    let processedChatHistory = chatHistory;
    if (chatHistory && Array.isArray(chatHistory) && chatHistory.length > CHAT_HISTORY_CONFIG.SUMMARIZATION_THRESHOLD) {
        console.log('🔄 [CHAT HISTORY] Summarizing chat history:', {
            originalLength: chatHistory.length,
            triggerReason: `History exceeds ${CHAT_HISTORY_CONFIG.SUMMARIZATION_THRESHOLD} messages`,
            sessionId: sessionId
        });
        processedChatHistory = await summarizeChatHistory(chatHistory, CHAT_HISTORY_CONFIG.KEEP_RECENT_MESSAGES, sessionId);
    }
    
    // Build conversation context using processed history
    let conversationContext = '';
    if (processedChatHistory && Array.isArray(processedChatHistory) && processedChatHistory.length > 0) {
        conversationContext = '\n\nCONVERSATION HISTORY:\n' + processedChatHistory.map(msg => 
            `${msg.role === 'user' ? 'User' : msg.role === 'assistant' ? 'Assistant' : 'System'}: ${msg.content || ''}`
        ).join('\n');
        
        console.log('📊 [TOKEN MANAGEMENT] Conversation history processed:', {
            originalLength: chatHistory.length,
            processedLength: processedChatHistory.length,
            conversationContextLength: conversationContext.length,
            wasSummarized: processedChatHistory.length < chatHistory.length
        });
    }
...
Respond naturally and helpfully based on the user's request.`;
        ...
        return await buildDetailedAnalysisPrompt(message, chatHistory, userProfile, jobContext, toggleState, sessionId);
    }
...
async function buildNaturalIntentPrompt(message, chatHistory, userProfile, jobContext, isProfileToggleOff, sessionId = null) {
  ...
  // ✅ NEW: Use summarized chat history instead of full history
  let processedChatHistory = chatHistory;
  if (chatHistory && Array.isArray(chatHistory) && chatHistory.length > CHAT_HISTORY_CONFIG.SUMMARIZATION_THRESHOLD) {
    console.log('🔄 [NATURAL INTENT] Summarizing chat history:', {
      originalLength: chatHistory.length,
      triggerReason: `History exceeds ${CHAT_HISTORY_CONFIG.SUMMARIZATION_THRESHOLD} messages`,
      sessionId: sessionId
    });
  ...
  if (processedChatHistory && processedChatHistory.length > 0) {
    conversationContext = '\n\nCONVERSATION HISTORY:\n' + processedChatHistory.map(function (msg) {
      return "".concat(msg.role === 'user' ? 'User' : msg.role === 'assistant' ? 'Assistant' : 'System', ": ").concat(msg.content);
    }).join('\n');
    
    // ✅ DEBUG: Log conversation history being included
    console.log('🔍 [CONVERSATION HISTORY DEBUG] Including processed conversation history:', {
      originalLength: chatHistory ? chatHistory.length : 0,
      processedLength: processedChatHistory.length,
      conversationContextLength: conversationContext.length,
      wasSummarized: processedChatHistory.length < (chatHistory ? chatHistory.length : 0),
      firstMessage: processedChatHistory[0] ? processedChatHistory[0].content.substring(0, 100) + '...' : 'none',
      lastMessage: processedChatHistory[processedChatHistory.length - 1] ? processedChatHistory[processedChatHistory.length - 1].content.substring(0, 100) + '...' : 'none'
    });
  } else {
...
// ✅ REMOVED: parseAIDecision function - no longer needed with single-call architecture
// Decision tags are now cleaned from responses automatically
...
async function buildDetailedAnalysisPrompt(message, chatHistory, userProfile, jobContext, toggleState, sessionId = null) {
    ...
    let processedChatHistory = chatHistory;
    if (chatHistory && Array.isArray(chatHistory) && chatHistory.length > CHAT_HISTORY_CONFIG.SUMMARIZATION_THRESHOLD) {
        console.log('🔄 [DETAILED ANALYSIS] Summarizing chat history:', {
            originalLength: chatHistory.length,
            triggerReason: `History exceeds ${CHAT_HISTORY_CONFIG.SUMMARIZATION_THRESHOLD} messages`,
            sessionId: sessionId
        });
