...
// Handle password reset verification (must come before verify-complete)
app.get('/auth/verify-complete', cors(SECURITY_CONFIG.cors), (req, res) => {
    const { token_hash, type } = req.query;
    
    console.log('Verify-complete route hit:', { token_hash: !!token_hash, type });
    
    if (type === 'recovery' && token_hash) {
        // Handle password reset - redirect to our password reset page
        console.log('Password reset verification received, redirecting to reset page');
        res.redirect(`/auth/reset-password?token_hash=${token_hash}&type=${type}`);
        return;
    }
    
    // For non-recovery types, continue to the original logic
    // This is a simplified version that only handles recovery
    if (type === 'signup' && token_hash) {
...
// Original email verification completion endpoint (now unused but kept for reference)
app.get('/auth/verify-complete-original', async (req, res) => {
    try {
        // Extract token and type from query parameters
        const { token_hash, type } = req.query;
...
// Handle Supabase's default verify route
app.get('/auth/v1/verify', cors(SECURITY_CONFIG.cors), (req, res) => {
    const { token_hash, type } = req.query;
    
    console.log('Supabase verify route received:', { token_hash, type });
